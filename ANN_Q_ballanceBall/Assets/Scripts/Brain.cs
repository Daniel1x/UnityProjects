...
public class Brain : MonoBehaviour
{
    ...
    // Zmienna zezwalająca na losowy wybór akcji.
    ...
    public float exploreRate = 50f;
    // Maksymalny procent wyboru losowej akcji.
    private float maxExploreRate = 100f;
    ...
    private float exploreDecay = 0.0005f;
    ...
    private void Start()
    {
        ...
        ann = new ArtificialNeuralNetwork(4, 2, 1, 6, 0.2);
    ...
    private void FixedUpdate()
    {
        ...
        // Jeśli zezwolono na losowy wybór akcji.
        if (enableRandom)
        {
            // Losowy wybór akcji.
        ...
        if (ball.GetComponent<BallState>().dropped)
        {
            ...
            for(int i = replayMemory.Count - 1; i >= 0; i--)
            {
                ...
                // Sprzężenie zwrotne
                double feedback;
                // Jeśli dane aktualnego stanu są ostatnie na liście (koniec danych) 
                // lub nagroda jest ujemna(piłka upadła, koniec sekwencji).
                ...
                else
                {
                    ...
                    // Sprzężenie jest równe wartości aktualnej nagrody powiększonej
                    // o najlepszą wartość Q pomnożoną przez współczynnik pomniejszający.
                    // Na podstawie równania Bellmana.
                    feedback = replayMemory[i].reward + discount * nextMaxQValue;
                }
                // Dla wybranej akcji zmieniona zostaje wartość odpowiedniego wyjścia sieci neuronowej
                // (dla aktualnych stanów) używając sprzężenia zwrotnego.
