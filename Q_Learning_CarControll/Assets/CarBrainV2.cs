...
public class CarBrainV2 : MonoBehaviour
{
    public float crashReward = -5f;
    ...
    private void Start()
    {
        ...
        // Konstruktor sieci.
        ann = new ANN2(5, 1, 2, 10, 0.025f);
        // Przyspieszenie czasu.
        Time.timeScale = TimeScale;
        // Wczytywanie wag.
        if (LoadTrainedWeights) ann.LoadWeightsFromFile("QCarV2Trained");
    }
    ...
    private void FixedUpdate()
    {
        ...
        qValues = (ann.CalculateOutputs(states));
        ...
        if (enableRandom)
        {
            ...
            if (UnityEngine.Random.Range(0f, 100f) < exploreRate)
                qValues[0] = UnityEngine.Random.Range(-1f, 1f);
        }
        ...
        if (maxQIndex == 0)
        {
            rotate = rotateMultiplier * qValues[0];
        }
        ...
        if (GetComponent<CarState>().crashed)
        {
            ...
            for (int i = replayMemory.Count - 1; i >= 0; i--)
            {
                ...
                actualOutputs = (ann.CalculateOutputs(replayMemory[i].states));
                ...
                if (i == replayMemory.Count - 1 || replayMemory[i].reward == crashReward)
                ...
                else
                {
                    ...
                    nextOutputs = (ann.CalculateOutputs(replayMemory[i + 1].states));
                    // Najlepsza wartość Q.
    ...
    {
        ann.SaveWeightsToFile("QV2 "+transform.name);
    }
